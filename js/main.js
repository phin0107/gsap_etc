...
function updateActiveFromViewport() {
    //문서 맨 아래에 가까운지 확인
    const isNearBottom = window.scollY + window.innerHeight >= document.documentElement.scrollHeight - 80;
...
document.querySelectorAll('.tit_card').forEach((card) => {
    ...
    //카드 위에서 마우스를 움직일때 기울기값을 계산
    card.addEventListener('mousemove', (e) => {
        const rect = card.getBoundingClientRect();
        //카드안에서 마우스가 가로로 몇 퍼센트 위치인지 계산
        const px = (e.clientX - rect.left) / rect.width;
        const py = (e.clientY - rect.top) / rect.height;
        //위라애 위치를 rotateX각도로 바꾼다.
        const rx = (py - 0.5) * -15;
        const ry = (px - 0.5) * 22;
...
//sticky박스의 이미지를 바로 바꾸면 딱딱하게 바뀌어서 살짝 줄였다가 다시 키우며 교체
...
//배경이미지를 살짝 흐리고 키워서 뒤로 밀리는 느낌을 만든다.
pinTl.to(pinBg, { filter: 'blur(12px)', scale: 1.06, duration: 1, ease: 'none' }, 0);
//사진마다 살짝 다른 각도와 giltch 효과를 줘서 디자인 갤러리처럼 겹쳐보이게 한다.
photos.forEach((photo, index) => {
    ...
    pinTl.add(() => {
        ...
        //0.4초 뒤에 glitch클래스를 제거해서 효과를 끝낸다.
...
// 가로 스크롤섹션에서 실제로 옆으로 움직여야 되는 거리를 계산
//track의 전체 너비에서 화면에 이미 보이는 너비를 빼면 이동거리만 남음
function totalWidth() {
...
//horizontal_section은 세로 스크롤을 가로 이동으로 바꿔 보여준다.
...
//각 case_panel안에서 텍스트는 순서대로, 이미지는 서로 다른 방향에서 등장
...
document.querySelectorAll('.magnet_zone').forEach((zone) => {
    ...
    zone.addEventListener('mouseleave',()=>{
        BigInt.style.transform=`translate(0,0)`;
    });
});
...
window.addEventListener('load',()=>{
    ...
    //document.fonts가 있는 브라우저 라면 폰트 로딩도 기다린다.
    if(document.fonts){
        //fonts가 모두 준비된 뒤 scrolltrigger 위치를 다시 맞춘다.
